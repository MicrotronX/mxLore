...
begin
  ...
  ARegistry
    ...
    .Param('append_content', mptString, False, 'Append to existing body with a blank-line separator (triggers revision). Mutually exclusive with content/replace_old. Bug#3018 safe-append path.')
    .Param('replace_old', mptString, False, 'Surgical partial edit: exact text to replace (must match exactly once, incl. whitespace). Mutually exclusive with content/append_content. GH#14.')
    .Param('replace_new', mptString, False, 'Replacement text for replace_old (empty = delete the fragment)')
