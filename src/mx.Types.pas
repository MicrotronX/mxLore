...
const
  MXAI_VERSION = '2.4.0';
  MXAI_BUILD   = 101;  // Build 101 (Session 264, FR#2936/Plan#3266 M3 Bundles 1+2+3 partial): sql/049 schema foundation (developers.accept_agent_messages BOOL + client_keys revoked_*/forensik trio/last_warned_stage + tool_call_log.auth_reason + active_prefix VIRTUAL UNIQUE for Bug#3199 belt-suspenders). M3.1 mx_agent_send asymmetric send-rules (admin->all, alReadWrite->same-project, alComment->admins+alReadWrite, alReadOnly->no-send) + size limits 1000 soft / 4000 hard. M3.2 accept_agent_messages opt-out + admin-priority-override + Self-Messaging exempt from rate-limit. M3.3 role-dependent default expires_at at key-creation (admin=180d, readwrite=90d, read=30d). M3.4 Grace-Period 24h read-only after expiry (downgrade to mpRead in mx.Auth.ValidateKey both PBKDF2+legacy paths) + revoked_at IS NULL filter. M3.6 POST /keys/:id/revoke admin-side endpoint with revoke_ip+revoke_user_agent+revoke_actor_type populate. M3.5 Rotation-API + M3.4 warning-cadence statusline + M3.9-M3.12 RFC7807/Degraded-Mode DEFERRED to next session.
  MX_KEY_PREFIX = 'mxk_';
