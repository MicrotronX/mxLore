...
cases = [
    ...
    ("allow", "git -C dir add", "git -C %s add %s" % (cwd, D), "Bash"),
    ("allow", "git -c k=v commit -m with glob text", "git -c core.safecrlf=false commit -q -m 'guard for *%s files'" % X, "Bash"),
    ("DENY", "git -C dir show", "git -C %s show HEAD:%s" % (cwd, X), "Bash"),
