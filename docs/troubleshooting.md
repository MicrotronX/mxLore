...
| **`-314` "cannot load vendor library … The specified module could not be found"** (even though `libmariadb.dll` exists) | This is **not** a missing `libmariadb.dll` — it is a missing **dependency** of that DLL. Windows error 126 (`ERROR_MOD_NOT_FOUND`) also fires when the DLL is found but one of *its* dependencies cannot be resolved. MariaDB server installs link `lib\libmariadb.dll` against OpenSSL 3 (`libcrypto-3-x64.dll`, `libssl-3-x64.dll`), which live in the sibling `bin\` folder — **not** in `lib\`. Windows does not search `bin\` automatically. **Fix:** add the MariaDB `bin\` directory (e.g. `C:\Program Files\MariaDB 12.3\bin`) to the **system PATH**, then restart the server process/service. **Alternative (self-contained):** copy `libmariadb.dll` **together with its `libcrypto-3-x64.dll` / `libssl-3-x64.dll` and `vcruntime140.dll`** into the mxLore `lib\` folder next to the EXE. Note: a working HeidiSQL on the same machine does **not** prove the DLL works for mxLore — HeidiSQL ships its own (often 32-bit) self-contained `libmariadb.dll` and does not load MariaDB's `lib\` copy. |
| **"Access denied" / "Zugriff verweigert"** | Windows HTTP.sys requires URL reservations. Run **once** as Administrator: `netsh http add urlacl url=http://+:8080/ user=Everyone` and `netsh http add urlacl url=http://+:8081/ user=Everyone`. After that, the server runs without admin rights. |
| **"Port 8080 already in use"** | Another application is using port 8080. Change `Port=8080` in the `[Server]` section to a free port (e.g. 9080). The Admin UI port is configured separately via `admin_port=` in the `[Admin]` section (default: 8081). |
